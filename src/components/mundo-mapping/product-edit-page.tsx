...
import { ProductEditor } from "@/components/mundo-mapping/product-editor";
import { useProductStore } from "@/components/mundo-mapping/product-store";

export function ProductEditPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!product) {
    return (
      <>
        <PageHeader
          description="Não encontramos esse produto para edição."
          eyebrow="Mundo Mapping / Afiliados / Produtos"
          title="Produto não encontrado"
        />
        <div className="p-6">
          <SectionCard subtitle="Volte ao dashboard ou crie um novo cadastro." title="Nada por aqui">
  ...
  return <ProductEditor initialProduct={product} mode="edit" />;
}
